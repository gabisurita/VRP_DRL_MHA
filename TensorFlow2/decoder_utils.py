...
class Env:
    def __init__(self, x, node_embeddings):
        ...
        self.visited_customer = tf.zeros(
            (self.batch, self.n_nodes - 1, 1), dtype=tf.bool
        )
    ...
    def get_mask_D(self, next_node, visited_mask, D):
        ...
        selected_demand = tf.gather(
            params=self.demand, indices=customer_idx, batch_dims=1
        )
    ...
    def _get_step(self, next_node, D):
        ...
        prev_node_embedding = tf.gather(
            self.node_embeddings, indices=next_node, batch_dims=1
        )
    ...
    def create_context_D_t1(self):
        ...
        depot_embedding = tf.gather(
            self.node_embeddings, indices=depot_idx, batch_dims=1
        )
    ...
    def get_costs(self, pi):
        ...
        return (
            ...
            + tf.norm(
                d[:, 0] - self.depot_xy, ord=2, axis=1
            )  # distance from depot to first selected node
            + tf.norm(
                d[:, -1] - self.depot_xy, ord=2, axis=1
