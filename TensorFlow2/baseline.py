...
class RolloutBaseline:
    ...
    def _update_baseline(self, model, epoch):
        ...
        if self.from_checkpoint and self.alpha == 0:
            ...
            self.model = copy_model(
                model, embed_dim=self.embed_dim, n_customer=self.n_customer
            )
            # For checkpoint
        ...
        self.dataset = generate_data(
            n_samples=self.n_rollout_samples, n_customer=self.n_customer
        )
    ...
    def ema_eval(self, cost):  # def eval
        ...
        if self.M is None:  # first iteration
            ...
            self.M = self.beta * self.M + (1.0 - self.beta) * tf.reduce_mean(cost)
        return self.M
    ...
    def eval_all(self, dataset, batch):
        ...
        val_costs = rollout(self.model, dataset, batch)
